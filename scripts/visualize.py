...
def animate_trading(day_data, infos, agents, stats, interval=1):
    ...
    for i in range(len(infos)):
        ...
        marker_line, = ax.plot([], [], linestyle='', marker=style, label=f'{agents[i]}', alpha=0.7, markersize=1 + len(infos)-i)
    ...
    for i in range(len(infos)):
        ...
        name = ax.text(1.02, name_y,   "", transform=ax.transAxes,
                    fontweight='bold', ha='left', va='top')
        qtxt = ax.text(1.02, q_y,      "", transform=ax.transAxes,
                    ha='left', va='top')
        orct = ax.text(1.02, orc_y,    "", transform=ax.transAxes,
                    ha='left', va='top')
        vwpt = ax.text(1.02, vwap_y,   "", transform=ax.transAxes,
                    ha='left', va='top')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--agents', nargs='+', type=str, help='A list of integers separated by spaces')
